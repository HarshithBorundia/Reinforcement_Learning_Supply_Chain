...
def phi_size(n_stores, type_of_phi, n_rbf=3):
    ...
    elif type_of_phi == 3:  # use laplace
        return n_rbf * (n_stores + 1) + 1
    else:
        return 1
...
def Laplace_vec(state, centers, sigma=50):
    lap_vec = zeros((centers.shape))  # shape of centers: nbr_lap, n_stores+1
    for i in range(centers.shape[0]):
        lap_vec[i, :] = exp(-abs(state - centers[i, :]) / sigma)
    return hstack([lap_vec[:, i] for i in range(lap_vec.shape[1])])
...
class REINFORCE_agent(object):
    ...
    def compute_phi(self, obs, type_of_phi, n_stores, n_rbf=3):
        ...
        if type_of_phi == 1:  # use squared stocks
            ...
            statemid = (statemax + statemin) / 2
            centers = array([statemin, statemid, statemax])
            lap = Laplace_vec(obs[:n_stores + 1], centers)
            return hstack(([1], obs, lap))
        else:
            return hstack(([1], obs))
    ...
    def choose_action(self, obs, allowed_actions, epsilon=0):  # returns one of the allowed actions
        ...
        count_allowed_a = int(sum(allowed_actions))
        prob = zeros(count_allowed_a)
        actions_ind = zeros(count_allowed_a)
        dotproduct = zeros(count_allowed_a)
        counter = 0
                    ...
                    for k in range(available_actions.shape[0]):
                        for l in range(available_actions.shape[0]):
                            self.discrete2continuous.append(
                                array([int(available_actions[l, 0]), int(available_actions[i, 1]),
                                       int(available_actions[j, 2]), int(available_actions[k, 3])]))
