...
for test_num in range(len(test_to_run)):
    if test_to_run[test_num]:
        ...
        if add_reinforce_4:
            result_file_names.append(test_name + "_reinforce_phi4")
            agents.append(REINFORCE_agent(env, actions_per_store=3, max_steps=max_steps, type_of_phi=3))
        if add_dqn:
            result_file_names.append(test_name + "_dqn")
            agents.append(DQN_agent(env, max_steps=max_steps))  # Adjust arguments as needed
